...
pub enum Source<'a> {
    ...
    Attachment(&'a fbo::Attachment, &'a Option<fbo::FramebuffersContainer>),
...
pub fn read<'s, S, D, T>(mut ctxt: &mut CommandContext, source: S, rect: &Rect, dest: D)
                         where S: Into<Source<'s>>, D: Into<Destination<'a, T>>
{
    ...
    let (fbo, read_buffer) = match source {
        Source::Attachment(attachment, framebuffer_objects) => {
            framebuffer_objects.as_ref().unwrap()
    ...
    unsafe {
        ...
        // reading
        match dest {
            Destination::Memory(buf) => {
                assert!(buf.len() >= pixels_to_read);
